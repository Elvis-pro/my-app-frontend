...
export default function MovieList() {
  ...
  const fetchMovies = async () => {
    try {
      const res = await fetch("http://localhost:3000/api/movies/getMostPopular");
  ...
  const fetchSeries = async () => {
    try {
      const res = await fetch("http://localhost:3000/api/series/getMostPopular");
      const data = await res.json();
      setSeries(data);
    } catch (error) {
  ...
  return (
    <div className="movie-section">
      ...
      <div className="filter-buttons">
        ...
        <button className="more-btn"  onClick={() => navigate("/moremostpopular")}>More</button>
      </div>
